...
template<typename... Ts>
class Group { //un peu deprecated, bon mais pas conceptuel enought
public:
    Group(Sparse<Ts>&... sparses) noexcept
        : storage_(sparses...) {
            std::cout << "Group created with : " << std::endl;
            (..., (std::cout << "> " << typeid(Sparse<Ts>).name() << std::endl));
    }

    template<typename F>
    auto each(F& lambda) noexcept ->
        std::enable_if_t<std::is_invocable_v<F, Ts...>, void> {
            std::apply([&](auto&&... args) {
                std::size_t min_size = std::min({ args.count()... }); // sfinae i guess
...
int main() {
    ...
    /*auto temp1 = Sparse<int>();
    auto temp2 = Sparse<float>();
    auto temp3 = Sparse<double>();
    Group<int, float, double> group(temp1, temp2, temp3);

    group.each([&](int a, float b, double c) {
        std::cout << "Lambda elements : "
            << typeid(a).name() << " "
            << typeid(b).name() << " "
    ...
    Sparse<Position> s_pos;
    s_pos.emplace_default(e0);
    s_pos.emplace_default(e1);
    s_pos.emplace_default(e3);

    Sparse<Velocity> s_vel;
    s_vel.emplace_default(e0);
    s_vel.emplace_default(e1);


    View<Position, Velocity> view(s_pos, s_vel);
